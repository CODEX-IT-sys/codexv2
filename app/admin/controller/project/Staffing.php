            ...
            foreach ($tr as $k => $v) {
                ...
                if (!empty($v['trtree'])) {
                    foreach ($v['trtree'] as $k1 => $v1) {
                        $v['trtree'][$k1]['title'] = "翻译_文件编号:$v1[file_code_project]\r" . "项目名称:".$v1['basic']['project_name']."\r" . "开始时间:$v1[tr_start_time]\r" . "结束时间:$v1[tr_end_time]";
                ...
                if (!empty($v['xdtree'])) {
                    foreach ($v['xdtree'] as $k2 => $v2) {
                        $v['trtree'][$k2]['title'] = "校对_文件编号:$v2[file_code_project]\r" . "项目名称:".$v1['basic']['project_name']."\r" . "开始时间:$v2[pr_start_time]\r" . "结束时间:$v2[pr_end_time]";
                ...
                if (!empty($v['yptree'])) {
                    foreach ($v['yptree'] as $k3 => $v3) {
                        $v['trtree'][$k3]['title'] = "预排_文件编号:$v3[file_code_project]\r" . "项目名称:".$v1['basic']['project_name']."\r" . "开始时间:$v3[be_start_time]\r" . "结束时间:$v3[be_end_time]";
                ...
                if (!empty($v['hptree'])) {
                    foreach ($v['hptree'] as $k4 => $v4) {
                        $v['trtree'][$k4]['title'] = "后排_文件编号:文件编号:$v4[file_code_project]\r" . "项目名称:".$v1['basic']['project_name']."\r" . "开始时间:$v4[after_start_time]\r" . "结束时间:$v4[after_end_time]";
