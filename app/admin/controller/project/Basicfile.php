...
/**
 * @ControllerAnnotation(title="项目文件")
 */
class Basicfile extends AdminController
{
    ...
    public function __construct(App $app)
    {
        ...
        $this->model = new \app\admin\model\project\Basicfile();
    ...
    public function index()
    {
        ...
        if ($this->request->isAjax()) {
            ...
            $count = $this->model
                ->where($where)
                ->withJoin(['label', 'basic',], 'LEFT')
                ->when($a, function ($query, $a) {
                    // 满足条件后执行
                    return $query->where('basic_id', $a);
                })
                ->count();
            $list = $this->model
                ->where($where)
        ...
        $this->assign(['basic_id' => $a]);
        return $this->fetch();
    }
    /**
     * @NodeAnotation(title="项目文件上传")
     */
    public function fileup($id)
    {
        ...
        //文件标签
        $labe = DatabaseContent::where('list_id', 13)->select()->toArray();
        $label = array();
        foreach ($labe as $k => $v) {
