   ...
   public static function onMessage($client_id, $data) {
       ...
       switch($message_type) {
           case 'init':
               ...
               $time = time() - 7 * 3600 * 24;
               $resMsg = $db1->select('id,fromid,fromname,fromavatar,timeline,content')->from('ea_chatlog')
               ...
               if( !empty( $resMsg ) ){
                   ...
                   foreach( $resMsg as $key=>$vo ){
                       ...
                       //设置推送状态为已经推送
                       $db1->query("UPDATE `ea_chatlog` SET `needsend` = '0' WHERE id=" . $vo['id']);

                   }
               }

               //查询当前的用户是在哪个分组中,将当前的链接加入该分组
               $ret = $db1->query("select `groupid` from `ea_groupdetail` where `userid` = {$uid} group by `groupid`");
               if( !empty( $ret ) ){
           ...
           case 'chatMessage':
               ...
               switch ($type) {
                   ...
                   case 'friend':
                       ...
                       $db1->insert('ea_chatlog')->cols( $param )->query();
